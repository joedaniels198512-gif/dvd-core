...
if ! "$OBJDUMP" -d "$BIN" >/tmp/mister_dvd_objdump.txt 2>/tmp/mister_dvd_objdump.err; then
    die "objdump failed: $(cat /tmp/mister_dvd_objdump.err)"
fi
if grep -E '[[:space:]](vfma|vfms|vfnma|vfnms)\.' /tmp/mister_dvd_objdump.txt >/dev/null; then
    echo "ERROR: VFPv4 instructions present — refusing $BIN" >&2
    grep -E '[[:space:]](vfma|vfms|vfnma|vfnms)\.' /tmp/mister_dvd_objdump.txt | head
    exit 1
fi
