...
#
# Build the first combined DVD A/V proof for the SS1.
#
# Same toolchain + CPU constraints as player/build_mac.sh:
#   arm-unknown-linux-gnueabihf-gcc, Cortex-A9 / NEON / VFPv3, no VFPv4.
#
# Reuses existing FFmpeg static libs (swscale + swresample) and
# dvdnav/dvdread. Does NOT rebuild FFmpeg.
#
...
mkdir -p "$OUT"
rm -f "$OUT/dvd_av_test"

echo "Building dvd_av_test..."
...
chmod +x "$OUT/dvd_av_test"

echo
echo "Built: $OUT/dvd_av_test"
