...
#
# Build, copy and run the MISTER_FB DDR writer on the SS1.
#
...
scp "$HERE/../dist/ddr_fb_writer" "$SS1:$REMOTE/ddr_fb_writer"

echo
echo "=== Running on SS1 ==="
ssh -t "$SS1" "
    chmod +x '$REMOTE/ddr_fb_writer'
    '$REMOTE/ddr_fb_writer'
    CODE=\$?
    echo
    echo EXIT CODE=\$CODE
"
