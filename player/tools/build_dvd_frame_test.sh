...
#
# Build the single-frame DVD -> MISTER_FB test for the SS1.
#
...
mkdir -p "$OUT"
rm -f "$OUT/dvd_frame_test"

echo "Building dvd_frame_test..."
...
"$CC" \
    ...
    -I"$DVDNAV_DEV" \
    -I"$DVDREAD_DEV" \
    -o "$OUT/dvd_frame_test" \
    "$HERE/dvd_frame_test.c" \
    "$DVDNAV_SO" \
    "$DVDREAD_SO" \
    -Wl,--start-group \
...
if "$OBJDUMP" -d "$OUT/dvd_frame_test" \
...
chmod +x "$OUT/dvd_frame_test"

echo
echo "Built: $OUT/dvd_frame_test"
