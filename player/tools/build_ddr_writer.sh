...
mkdir -p "$OUT"
rm -f "$OUT/ddr_fb_writer"

echo "Building ddr_fb_writer..."
...
"$CC" \
    ...
    $COMMON_FLAGS \
    -o "$OUT/ddr_fb_writer" \
    "$HERE/ddr_fb_writer.c"

echo "Checking binary for VFPv4 fused instructions..."

if "$OBJDUMP" -d "$OUT/ddr_fb_writer" \
...
chmod +x "$OUT/ddr_fb_writer"

echo
echo "Built: $OUT/ddr_fb_writer"
