...
#
# Build, copy and run the continuous-video DVD -> MISTER_FB throughput test on the SS1.
#
# PREREQUISITES:
...
echo "=== Building ==="
"$HERE/build_dvd_video_test.sh"

echo
echo "=== Copying to SS1 ==="
ssh "$SS1" "mkdir -p '$REMOTE'"
scp "$HERE/../dist/dvd_video_test" "$SS1:$REMOTE/dvd_video_test"

echo
echo "=== Running on SS1 ==="
ssh -t "$SS1" "
    chmod +x '$REMOTE/dvd_video_test'
    LD_LIBRARY_PATH=/media/fat/DVD/lib \
    '$REMOTE/dvd_video_test' /dev/sr0
    CODE=\$?
    echo
    echo EXIT CODE=\$CODE
"
