...
#
# Build the PTS-paced mailbox double-buffer DVD test for the SS1.
#
...
mkdir -p "$OUT"
rm -f "$OUT/dvd_video_double_test"

echo "Building dvd_video_double_test..."
...
"$CC" \
    ...
    -I"$DVDNAV_DEV" \
    -I"$DVDREAD_DEV" \
    -o "$OUT/dvd_video_double_test" \
    "$HERE/dvd_video_double_test.c" \
    "$DVDNAV_SO" \
    "$DVDREAD_SO" \
    -Wl,--start-group \
...
if "$OBJDUMP" -d "$OUT/dvd_video_double_test" \
...
chmod +x "$OUT/dvd_video_double_test"

echo
echo "Built: $OUT/dvd_video_double_test"
